...
public class ParticleManager {
	...
	// Added the explode attributes for when the star's lifetime ends
	private static final double EXPLODE_VELOCITY = 				20.0;	//m/sec
	private static final double EXPLODE_LIFETIME = 				0.4;	// seconds
	private static final double EXPLODE_VELOCITY_VAR =			150.0;
	private static final int 	NUM_EXPLODE =			 		40;
	...
	private Spark starSparkTemplate;
	private Particle explodeTemplate;
	...
	public ParticleManager(double windVelocity, double launchAngle) throws EnvironmentException, EmitterException {
		...
		starSparkTemplate = new Spark(STAR_SPARK_LIFETIME, SPARK_MASS, SPARK_RADIUS, STAR_COLOURS[0]);
		explodeTemplate = new Particle(EXPLODE_LIFETIME, SPARK_MASS, SPARK_RADIUS, STAR_COLOURS[0]);
	...
	public void start(double time) throws EmitterException {
		...
		explodeTemplate.setColour(theStar.getColour());
		...
		explodeEmitter = new MobileEmitter(EXPLODE_VELOCITY, EXPLODE_VELOCITY_VAR, 0, STAR_SPARK_ANGLE_VARIATION,
				NUM_EXPLODE, explodeTemplate, theStar);
		countStars++;
	...
	private void update(double time) {
		...
		if (theStar.isAlive(time)){
			...
			fireworks.addAll(explodeEmitter.launch(time));
			countStars++;
		}
		else
			Main.isStart();		// Simulation is over
	...
	public void setTipPosition(double[] position) {
		tip = position.clone();
		launchTube.setPosition(tip);
		delaySparkEmitter.setPosition(tip);
		streakTemplate.setOrigin(tip);
		streakEmitter.setPosition(tip);
	} // end setTipPosition

	public double[] getTipPosition() { return tip.clone(); }

	private void explode(double time){
		fireworks.addAll(explodeEmitter.launch(time));
		fireworks.remove(theStar);
		try {
			start(time);
	...
	public void collision(double time){
		if (countStars < NUM_STARS)
			explode(time);
		else{
			int i;
			theStar.setLifetime(0.1);
			for (i = 0; i < fireworks.size(); i++)
				fireworks.remove(i);
			countStars++;
			fireworks.addAll(explodeEmitter.launch(time));
